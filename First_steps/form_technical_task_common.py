...
def pivot(file_name):  # создаёт списки сводных таблиц для каждого грузополучателя, в соответствии с бюджетом
    ...
    year_month = ('2019/12', '2020/01', '2020/02', '2020/03', '2020/04', '2020/05', '2020/06', '2020/07', '2020/08',
                  '2020/09', '2020/10', '2020/11', '2020/12')  # годы/месяцы поставки
    ...
    data_pt = pd.pivot_table(data, index=['Раздел_ГКПЗ', 'Завод', 'Номер лота', '№ материала', 'Краткий текст позиции',
                             ...
                             aggfunc=np.sum).sort_values(by=['Завод', 'Краткий текст позиции'])  # формируем общую сводную таблицу
    df = []
    for budget in budgets:  # создаём отдельные сводные таблицы для каждого завода и раздела ГКПЗ
        temp_pivot = data_pt.query(f'Раздел_ГКПЗ == ["{budget}"]')
...
class FormTechTaskComm:
    ...
    def __init__(self, some_table, some_list):
        ...
        self.final_wb = xl.Workbook(f'ТЗ_{self.budget_name}_Общее.xlsx')  # создаём конечный excel-файл, в который будем записывать данные
        self.final_ws = self.final_wb.add_worksheet(f'{self.budget_name}')  # добавляем лист, в который будем записывать данные
    ...
    def make_middle(self, lst_data):  # добавляем данные в таблицу 1 ТЗ
        r_num = 8
        ...
        for row in lst_data:
            ...
            self.final_ws.write_number(f'A{r_num}', row[0], format1)
            self.final_ws.write_row(f'B{r_num}', row[2:7], format1)
            ...
            if row[0] == self.some_list[k]:
                self.final_ws.merge_range(r_num, 0, r_num, 5, totals[f'{factory_id}'], format_total)
                j = 0
                col = 6
                while col <= 19:
                    self.final_ws.write_formula(r_num, col, f'=SUM({cells[j]}{r_num - self.some_list[k] + 1}:{cells[j]}{r_num})',
                                                format_total_num)
                    j += 1
                    col += 1
                r_num += 1
            ...
            r_num += 1
        self.final_ws.merge_range(r_num - 1, 0, r_num - 1, 5, 'Общий итог', format_total)
        x = 0
        y = 6
        while y <= 19:
            if len(self.some_list) == 5:
                self.final_ws.write_formula(r_num - 1, y, f'={cells[x]}{8 + self.some_list[0]}+'
                                            ...
                                            format_total_num)
            elif len(self.some_list) == 4:
                self.final_ws.write_formula(r_num - 1, y, f'={cells[x]}{8 + self.some_list[0]}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + 1}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + self.some_list[2] + 2}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + self.some_list[2] + self.some_list[3] + 3}+',
                                            format_total_num)
            elif len(self.some_list) == 3:
                self.final_ws.write_formula(r_num - 1, y, f'={cells[x]}{8 + self.some_list[0]}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + 1}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + self.some_list[2] + 2}+',
                                            format_total_num)
            elif len(self.some_list) == 2:
                self.final_ws.write_formula(r_num - 1, y, f'={cells[x]}{8 + self.some_list[0]}+'
                                                          f'{cells[x]}{8 + self.some_list[0] + self.some_list[1] + 1}+',
                                            format_total_num)
            else:
                self.final_ws.write_formula(r_num - 1, y, f'={cells[x]}{8 + self.some_list[0]}', format_total_num)
            x += 1
            y += 1
        self.final_ws.write(f'U{r_num}', None, format_total)
    ...
    def make_tail(self, row_num, factory_id):  # Вставляем таблицу 2 ТЗ
        ...
        if len(factory_id) == 5:
            self.final_ws.merge_range(f'E{row_num + 3}:U{row_num + 3}',
                                      ...
                                      f'{self.addresses[factory_id[4]]}', merge_format3)
            self.final_ws.set_row(row_num + 2, 126)
        elif len(factory_id) == 4:
            self.final_ws.merge_range(f'E{row_num + 3}:U{row_num + 3}',
                                      'Поставка осуществляется путем отгрузок продукции автомобильным транспортом силами'
                                      ' и за счет Поставщика до склада Грузополучателя по адресу:\n'
                                      f'{self.addresses[factory_id[0]]}\n{self.addresses[factory_id[1]]}\n'
                                      f'{self.addresses[factory_id[2]]}\n{self.addresses[factory_id[3]]}'
                                      , merge_format3)
            self.final_ws.set_row(row_num + 2, 106)
        elif len(factory_id) == 3:
            self.final_ws.merge_range(f'E{row_num + 3}:U{row_num + 3}',
                                      'Поставка осуществляется путем отгрузок продукции автомобильным транспортом силами'
                                      ' и за счет Поставщика до склада Грузополучателя по адресу:\n'
                                      f'{self.addresses[factory_id[0]]}\n{self.addresses[factory_id[1]]}\n'
                                      f'{self.addresses[factory_id[2]]}', merge_format3)
            self.final_ws.set_row(row_num + 2, 86)
        elif len(factory_id) == 2:
            self.final_ws.merge_range(f'E{row_num + 3}:U{row_num + 3}',
                                      'Поставка осуществляется путем отгрузок продукции автомобильным транспортом силами'
                                      ' и за счет Поставщика до склада Грузополучателя по адресу:\n'
                                      f'{self.addresses[factory_id[0]]}\n{self.addresses[factory_id[1]]}'
                                      , merge_format3)
            self.final_ws.set_row(row_num + 2, 66)
        else:
            self.final_ws.merge_range(f'E{row_num + 3}:U{row_num + 3}',
                                      'Поставка осуществляется путем отгрузок продукции автомобильным транспортом силами'
                                      ' и за счет Поставщика до склада Грузополучателя по адресу:\n'
                                      f'{self.addresses[factory_id[0]]}', merge_format3)
    ...
    def form(self):  # формируем ТЗ
        ...
        self.make_tail(curr_row_num[0] + 2, tuple(curr_row_num[1]))
        self.temp_wb.close()
        self.final_wb.close()
...
if __name__ == "__main__":
    ...
    for table in tables:
        ...
        for factory in factories:
            quantity = table.query(f'Завод == ["{factory}"]').shape[0]
            if quantity > 0:
                quantity_factory.append(quantity)
        quantity_budget.append(quantity_factory)
    count = 0
    while count < len(tables):
        FormTechTaskComm(tables[count], quantity_budget[count]).form()
        count += 1
